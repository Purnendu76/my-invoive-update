...
const Dashboard2 = () => {
  ...
  // Overdue by aging period calculation
  const overdueByProject = useMemo<OverdueBarChartData[] | null>(() => {
    ...
    const projectFiltered = project !== 'All Projects' 
      ? filtered.filter((inv: any) => inv.project === project)
      : filtered;
    
    // Count invoices for each aging period by both dates
    const agingCounts: Record<string, { invoiceDate: number; submissionDate: number }> = {};
    agingPeriods.forEach(period => {
      agingCounts[period.label] = { invoiceDate: 0, submissionDate: 0 };
    });
    
    projectFiltered.forEach((inv: any) => {
      // Calculate aging based on invoiceDate
      const invDate = inv.invoiceDate ? new Date(inv.invoiceDate).getTime() : null;
      if (invDate && !isNaN(invDate)) {
        const ageInMs = now - invDate;
        agingPeriods.forEach(period => {
          if (ageInMs >= period.ms) {
            agingCounts[period.label].invoiceDate++;
          }
        });
      }
      
      // Calculate aging based on submissionDate
      ...
      if (subDate && !isNaN(subDate)) {
        ...
        agingPeriods.forEach(period => {
          if (ageInMs >= period.ms) {
            agingCounts[period.label].submissionDate++;
          }
        });
      }
    });
